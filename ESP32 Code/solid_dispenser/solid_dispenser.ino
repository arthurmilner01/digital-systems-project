...
const float MAX_DISTANCE = 16.00; //For calculating capacity % (Height from ultrasonic to bottom of container)
const float LOW_CAPACITY = 12.00; //Triggers LED if capacity hits this distance (<25%)
...
void loop() 
{
  ...
  while(true)
  {
    if(scale.is_ready())
    {
      amountDispensed = scale.get_units(10);
    }
    costDispensed = amountDispensed * COST_PER_GRAM; //Cost
    ...
    lcd.setCursor(0,1);
    lcd.print("Cost:");
    lcd.print(String(costDispensed/100));
  ...
  if(amountDispensed < 0.01 || amountDispensed > 5 || costDispensed < 0.01 || costDispensed > 200)
  {
